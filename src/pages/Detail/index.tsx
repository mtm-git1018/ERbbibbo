import { useGetHospitalLocation } from "@/domain/er-status/api/useGetHospitalLocation";
...
function Detail() {
  ...
  const { data: locationData } = useGetHospitalLocation(hpid)

  
  const lat = locationData?.map((v) => v.wgs84Lat).join("");
  const long = locationData?.map((v) => v.wgs84Lon).join("");
