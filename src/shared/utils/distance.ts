 ...
 */
export function calculateSimpleDistance(coord1: Coordinates, coord2: Coordinates): number {
...
export function filterByRadius<T extends { coordinates: Coordinates }>(
  ...
  return locations.filter(location => 
    calculateDistance(userLocation, location.coordinates) <= radiusKm
  );
}
 ...
 * 문자열 좌표를 숫자로 변환하는 함수
 */
export function parseCoordinates(lat: string | number, lon: string | number): Coordinates | null {
